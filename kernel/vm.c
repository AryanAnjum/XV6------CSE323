...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  // Flush TLB since we modified the parent's PTEs (which are likely active)
  sfence_vma(); 
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    if((*pte & PTE_W) == 0) {
        if(*pte & PTE_COW) {
            // It's a CoW page. Trigger allocation.
            if(cowalloc(pagetable, va0) < 0)
                return -1;
            // Re-fetch pte as cowalloc might have remapped it? 
            // Actually cowalloc modifies *pte in place, so *pte is valid. 
            // But PA changed. We should re-fetch PA.
            pa0 = PTE2PA(*pte); 
        } else {
             // True Read-Only page
             return -1;
    ...
    // Re-lookup pa0 in case cowalloc remapped the page
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0) return -1;
...
// Allocate a new page for a CoW page fault.
...
cowalloc(pagetable_t pagetable, uint64 va)
{
  ...
  // Make the new page Writable, and NOT CoW
  flags |= PTE_W;
  flags &= ~PTE_COW;
